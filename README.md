...
- Parsing strategy: PDF text-layer first (when available), otherwise image rendering + Gemini Vision
...
이 프로젝트는 **임대차 계약서(한국)**에서 주요 항목을 추출하고, 사용자가 입력한 체크리스트 값과 일치/불일치 여부를 검증하는 **개념증명(POC)** Streamlit 웹 앱입니다.  
...
**범위**: 모델 학습용 저장소가 아닙니다. 문서 AI 관점의 오케스트레이션, 검증 로직, 안전 라우팅 패턴을 보여주는 POC입니다.

---

## 무엇을 하나요?
- 주요 항목 추출: 주소, 보증금, 월세, 계약기간, 임대인/임차인 성명, 주민등록번호(**필요 시 마스킹/주의 처리**)
- **문서 값 vs 체크리스트 값** 비교 후 일치/불일치 결과 표시(✅/❌)
- 디버그 출력 옵션 제공(**활성화 시 PII 포함 가능**)
- 불확실/고위험 케이스는 자동 확정하지 않고 **Human Review**로 라우팅
...
## 주요 기능(개요)
- 체크리스트 입력 + 계약서 업로드(PNG/JPG/PDF)
- 파싱 전략: PDF 텍스트 레이어 우선(가능한 경우), 불가 시 이미지 렌더링 + Gemini Vision
...
| 파일 | 역할 |
|------|------|
| **app.py** | Streamlit UI: 입력, 업로드, 결과 테이블 렌더링 |
| **ocr_service.py** | PDF 처리(텍스트 레이어 우선/이미지 폴백), Vision 호출, 조건부 정밀 검증 |
| **document_logic.py** | JSON 파싱/정규화, 최종 아이템 구성, 교차검증 로직 |
| **comparators.py** | 주소/금액/날짜/성명/식별정보 비교 유틸 및 포맷터 |
| **pii_mask.py** | PII 마스킹/정규화 유틸 |
| **check_models.py** | 환경/모델 점검 및 헬퍼 |
...
- 불확실/고위험 케이스는 자동 확정하지 않고 Human Review로 라우팅되도록 동작을 확인했습니다.
- 판단 근거를 로컬 로그로 남길 수 있도록 구성했습니다(커밋 금지).
